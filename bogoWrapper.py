...
def trainHAM(trainable_words, bogoDIR=os.getcwd()+"/.bogofilter"):
    ...
    #-n is for ham
    p2 = sp.call(["bogofilter", "-e", "-n", "-d", bogoDIR, "-c", bogoDIR+"/bogofilter.conf"], stdin=p1.stdout)
...
def trainSPAM(trainable_words, bogoDIR=os.getcwd()+"/.bogofilter"):
    ...
    #-e is for emmbeded, makes return codes simple
    #-s is for spam
    p2 = sp.call(["bogofilter", "-e", "-s", "-d",  bogoDIR, "-c", bogoDIR+"/bogofilter.conf"], stdin=p1.stdout)
    p1.stdout.close()  # p1 SIGPIPE exit
...
def bogofilter(words, bogoDIR=os.getcwd()+"/.bogofilter"):
    ...
    p2 = sp.call(["bogofilter", "-d", bogoDIR, "-c", bogoDIR+"/bogofilter.conf"], stdin=p1.stdout)
    p1.stdout.close()  # p1 SIGPIPE exit
